...
trait SmallBinType: Into<usize> + Sync + Send + Copy {
    const size: usize;

    fn from(it: impl Iterator<Item=bool>) -> Self;
}
...
pub fn binary_msm<F, G: CurveGroup<ScalarField=F>, T: SmallBinType>(coefs: &[T], bases: &[Vec<G::MulBase>]) -> G
where
    G::MulBase: Send,
{
...
pub fn prepare_chunk<F, G: CurveGroup<ScalarField=F>, T: SmallBinType>(chunk: &[G::MulBase]) -> Vec<G::MulBase> {
    let proj = (1..(1 << T::size)).map(
        |i| {
            (0..T::size).zip(chunk.iter().rev())
                .filter(|(idx, _)| (1 << idx) & i != 0)
                .map(|(_, b): (_, &G::MulBase)| *b)
...
pub fn prepare_bases<F, G: CurveGroup<ScalarField=F>, T: SmallBinType>(bases: &[G::MulBase]) -> Vec<Vec<G::MulBase>> {
...
pub fn prepare_coefs<T: SmallBinType>(coefs: impl Iterator<Item=bool>) -> Vec<T> {
    coefs.chunks(T::size).into_iter().map(|chunk| T::from(chunk)).collect()
}
...
mod test {
    ...
    fn bin_msm() {
        ...
        let coefs = (0..num).map(|i| gen.gen_bool(0.5)).collect_vec();
