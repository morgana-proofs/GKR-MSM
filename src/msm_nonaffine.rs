...
pub trait VariableBaseMSM_Nonaffine: ScalarMul {
  ...
  fn msm_unchecked_nonaff(bases: &[Self], scalars: &[Self::ScalarField]) -> Self {
    let bigints = ark_std::cfg_into_iter!(scalars)
      .map(|s| s.into_bigint())
      .collect::<Vec<_>>();
    Self::msm_bigint_nonaff(bases, &bigints)
  }
  ...
  fn msm_nonaff(bases: &[Self], scalars: &[Self::ScalarField]) -> Result<Self, usize> {
    (bases.len() == scalars.len())
      .then(|| Self::msm_unchecked_nonaff(bases, scalars))
      .ok_or_else(|| bases.len().min(scalars.len()))
  }

  /// Optimized implementation of multi-scalar multiplication.
  fn msm_bigint_nonaff(
    bases: &[Self],
    bigints: &[<Self::ScalarField as PrimeField>::BigInt],
  ) -> Self {
    if Self::NEGATION_IS_CHEAP {
      msm_bigint_wnaf_nonaff(bases, bigints)
    } else {
      msm_bigint_nonaff(bases, bigints)
    }
  }

  /// Streaming multi-scalar multiplication algorithm with hard-coded chunk
  /// size.
  fn msm_chunks_nonaff<I: ?Sized, J>(bases_stream: &J, scalars_stream: &I) -> Self
  where
    I: Iterable,
    ...
    J::Item: Borrow<Self>,
  {
    ...
    for _ in 0..(scalars_stream.len() + step - 1) / step {
      ...
      let scalars_step = (&mut scalars)
        ...
        .collect::<Vec<_>>();
      result += Self::msm_bigint_nonaff(bases_step.as_slice(), scalars_step.as_slice());
    }
    result
  }
}
