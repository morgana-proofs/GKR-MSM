...
// //                     Multi(_) => panic!("Unexpected multi-evaluation claim."),
// //                 };
// //
...
// // ==============================================================
// 
// pub trait GKRComponentRegistry<F: PrimeField>: Protocol<F, Trace=Vec<Self::TraceRow>, ClaimsToReduce=<Self as Protocol<F>>::ClaimsNew> {
//     type Layer: ComponentLayer<F>;
//     type TraceRow;
//     fn layer_wtns(layer: &Self::Layer, num_vars: usize, input: Vec<FragmentedPoly<F>>) -> (Vec<Self::TraceRow>, Vec<FragmentedPoly<F>>);
// }
// 
...
// }
// 
// pub struct BintreeParams<F: PrimeField, R: GKRComponentRegistry<F>> {
//     pub layers: Vec<R::Layer>,
//     pub num_vars: usize,
//     pub phantom_data: PhantomData<F>
// }
// 
// impl<F: PrimeField, R: GKRComponentRegistry<F>> BintreeParams<F, R> {
...
// }
// 
// pub struct BintreeProtocol<F: PrimeField, R: GKRComponentRegistry<F>> {
//     _marker: PhantomData<(F, R)>,
// }
// 
// pub struct BintreeProver<F: PrimeField, R: GKRComponentRegistry<F>> {
...
// }
// 
// pub struct BintreeVerifier<F: PrimeField, R: GKRComponentRegistry<F>> {
...
// }
// 
// impl<F: PrimeField, R: GKRComponentRegistry<F>> Protocol<F> for BintreeProtocol<F, R> {
//     type Prover = BintreeProver<F, R>;
// 
//     type Verifier = BintreeVerifier<F, R>;
// 
...
// }
// 
// impl<F: PrimeField, R: GKRComponentRegistry<F>> ProtocolProver<F> for BintreeProver<F, R> {
//     type ClaimsToReduce = MultiEvalClaim<F>;
// 
//     type ClaimsNew = EvalClaim<F>;
// 
...
//                 *current_prover = Some(
//                     R::Prover::start(current_claims.take().unwrap(), vec![current_trace], &(current_params, current_num_vars))
//                 )
//             },
//             Some(_) => (),
//         };
// 
//         match current_prover.as_mut().unwrap().round(challenge, transcript) {
//             None => {}
//             Some((claim, proof)) => {
...
// }
// 
// impl<F: PrimeField, R: GKRComponentRegistry<F>> ProtocolVerifier<F> for BintreeVerifier<F, R> {
//     type Params = BintreeParams<F, R>;
// 
//     type ClaimsToReduce = MultiEvalClaim<F>;
// 
...
//         ).collect_vec();
// 
//         let params: BintreeParams<Fr, BintreeAddComponent<Fr>> = BintreeParams::new(
//             vec![
